...
async function addCustomers() {
  try {
    ...
    const { result : { customer } } = await customersApi.createCustomer({
      ...
      givenName: "John",
      familyName: "Doe",
      emailAddress: "clinedog8@gmail.com" // it is a fake email
    });
    ...
    await customersApi.createCustomer({
      ...
      givenName: "Amelia",
      familyName: "Earhart",
      emailAddress: "clinegary97@gmail.com" // it is a fake email
    });

    console.log("Successfully registered sugarbabies");
  } catch (error) {
    console.error("Register sugarbabies failed: ", error);
  }
}
...
async function clearCustomers() {
  try {
    ...
    if (customers) {
      ...
      console.log("Successfully deleted sugarbabies");
    } else {
      console.log("No sugarbabies to delete");
    }
  } catch (error) {
    console.error("Error in deleting sugarbabies:", error);
  }
}
...
async function disableSubscriptionPlans() {
  ...
  try {
    ...
    if (objects && objects.length > 0) {
      ...
      console.log("Successfully disabled all deposit enrollments.");
    } else {
      console.log("No deposit enrollments to disable from catalog");
    }
  } catch (error) {
...
program
  .command("clear")
  .description("clear all sugarbabies and disable all deposit enrollments in your sandbox.")
  .action(() => {
    const rl = readline.createInterface({
      input: process.stdin,
      output: process.stdout,
    });
    rl.question("Are you sure you want to clear all sugarbabies and disable all deposit entollments in your sandbox? (y/N) ", async (ans) => {
...
program
  .command("generate")
  .description("create three deposit enrollments and register two sugarbabies, one with card on file.")
  .action(async () => {
    await addCustomers();
